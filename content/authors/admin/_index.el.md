---
# Display name
title: Δρ. Ευάγγελος Βλάχος

# Full name (for SEO)
first_name: Ευάγγελος
last_name: Βλάχος

# Is this the primary user of the site?
superuser: true

# Role/position
role: Εντεταλμένος Ερευνητής (Ερευνητής Γ')

# Organizations/Affiliations
organizations:
  - name: Ινστιτούτο Βιομηχανικών Συστημάτων (ΙΝΒΙΣ), ΕΚ «Αθηνά»
    url: https://www.isi.gr/

# Short bio (displayed in user profile at end of posts)
bio: Η έρευνά μου εστιάζει στα Κυβερνο-Φυσικά Συστήματα, τα UAVs και τα δίκτυα 6G, γεφυρώνοντας τη θεωρία με την πράξη.

# Interests
interests:
  - Ενιαία Επικοινωνία, Έλεγχος και Ανίχνευση (JC&S)
  - Ολοκληρωμένη Ανίχνευση & Επικοινωνίες (ISAC) - 6G
  - Σμήνη UAV & Ανθεκτικότητα U-Space
  - Scientific Machine Learning (Deep Unfolding)

education:
  courses:
    - course: Διδακτορικό στις Ασύρματες Επικοινωνίες
      institution: Πανεπιστήμιο Πατρών
      year: 2015
    - course: Μεταπτυχιακό (M.Sc.) στην Επεξεργασία Πληροφορίας
      institution: Πανεπιστήμιο Πατρών
      year: 2009
    - course: Δίπλωμα Μηχανικού Η/Υ και Πληροφορικής
      institution: Πανεπιστήμιο Πατρών
      year: 2005

# Social/Academic Links (Μένουν ίδια)
social:
  - icon: envelope
    icon_pack: fas
    link: 'mailto:evlachos@athenarc.gr'
  - icon: google-scholar
    icon_pack: ai
    link: https://scholar.google.com/citations?user=YOUR_ID
  - icon: linkedin
    icon_pack: fab
    link: https://www.linkedin.com/in/evangelos-vlachos-b543103a/
  - icon: github
    icon_pack: fab
    link: https://github.com/vlaxose

# Link to a PDF of your resume/CV
url_pdf: "uploads/resume.pdf"
---

Ο Δρ. Ευάγγελος Βλάχος είναι Εντεταλμένος Ερευνητής στο ΙΝΒΙΣ του ΕΚ «ΑΘΗΝΆ», με ερευνητικό αντικείμενο την Ενιαία Επικοινωνία, Έλεγχο και Ανίχνευση (JC&S).

Εστιάζοντας στα δίκτυα 6G και τα Κυβερνο-Φυσικά Συστήματα, επιδιώκει τη σύνδεση της θεμελιώδους έρευνας με τις πρακτικές εφαρμογές, δοκιμάζοντας θεωρητικά μοντέλα σε πραγματικά περιβάλλοντα.

Έχει διατελέσει Επιστημονικός Υπεύθυνος σε ερευνητικά έργα (όπως τα EUSOME και SPLASH, ύψους >850k€), ενώ το δημοσιευμένο έργο του περιλαμβάνει 47 εργασίες με περισσότερες από 1.100 αναφορές. Είναι Visiting Researcher στο University of Sussex, με προηγούμενη ερευνητική εμπειρία στο University of Edinburgh και στο Πανεπιστήμιο Πατρών.
